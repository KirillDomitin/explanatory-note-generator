...
def get_participants(data: dict) -> list:
    ...
    if not isinstance(participants, list):
        name = participants.get("СвФЛ").get("@attributes", {}).get("Имя", "")
        surname = participants.get("СвФЛ").get("@attributes", {}).get("Фамилия", "")
        patronymic = participants.get("СвФЛ").get("@attributes", {}).get("Отчество", "")
        stock_share = participants.get("ДоляУстКап", {}).get("РазмерДоли", {}).get("Процент", "")
